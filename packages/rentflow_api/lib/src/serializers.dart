//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_import

import 'package:one_of_serializer/any_of_serializer.dart';
import 'package:one_of_serializer/one_of_serializer.dart';
import 'package:built_collection/built_collection.dart';
import 'package:built_value/json_object.dart';
import 'package:built_value/serializer.dart';
import 'package:built_value/standard_json_plugin.dart';
import 'package:built_value/iso_8601_date_time_serializer.dart';
import 'package:rentflow_api/src/date_serializer.dart';
import 'package:rentflow_api/src/model/date.dart';

import 'package:rentflow_api/src/model/add_image_request.dart';
import 'package:rentflow_api/src/model/client_flow_metrics_response.dart';
import 'package:rentflow_api/src/model/complete_avito_oauth_request.dart';
import 'package:rentflow_api/src/model/complete_import_request.dart';
import 'package:rentflow_api/src/model/confirm_email_change_request.dart';
import 'package:rentflow_api/src/model/confirm_phone_change_request.dart';
import 'package:rentflow_api/src/model/connect_platform_request.dart';
import 'package:rentflow_api/src/model/connection_response.dart';
import 'package:rentflow_api/src/model/create_additional_service_request.dart';
import 'package:rentflow_api/src/model/create_authorization_request.dart';
import 'package:rentflow_api/src/model/create_contact_note_request.dart';
import 'package:rentflow_api/src/model/create_contact_request.dart';
import 'package:rentflow_api/src/model/create_deal_meeting_request.dart';
import 'package:rentflow_api/src/model/create_deal_message_request.dart';
import 'package:rentflow_api/src/model/create_deal_note_request.dart';
import 'package:rentflow_api/src/model/create_deal_request.dart';
import 'package:rentflow_api/src/model/create_deal_task_request.dart';
import 'package:rentflow_api/src/model/create_document_request.dart';
import 'package:rentflow_api/src/model/create_invoice_request.dart';
import 'package:rentflow_api/src/model/create_note_request.dart';
import 'package:rentflow_api/src/model/create_property_request.dart';
import 'package:rentflow_api/src/model/create_rental_request.dart';
import 'package:rentflow_api/src/model/create_session_request.dart';
import 'package:rentflow_api/src/model/create_signature_request.dart';
import 'package:rentflow_api/src/model/create_studio_lead_request.dart';
import 'package:rentflow_api/src/model/create_task_request.dart';
import 'package:rentflow_api/src/model/create_template_request.dart';
import 'package:rentflow_api/src/model/create_transaction_request.dart';
import 'package:rentflow_api/src/model/dashboard_event_entity_response.dart';
import 'package:rentflow_api/src/model/dashboard_event_response.dart';
import 'package:rentflow_api/src/model/document_response.dart';
import 'package:rentflow_api/src/model/document_signature_response.dart';
import 'package:rentflow_api/src/model/document_template_response.dart';
import 'package:rentflow_api/src/model/exchange_token_request.dart';
import 'package:rentflow_api/src/model/external_listing_response.dart';
import 'package:rentflow_api/src/model/finance_metrics_response.dart';
import 'package:rentflow_api/src/model/generate_document_upload_url_request.dart';
import 'package:rentflow_api/src/model/generate_document_upload_url_response.dart';
import 'package:rentflow_api/src/model/generate_upload_url_body_dto.dart';
import 'package:rentflow_api/src/model/get_me_response.dart';
import 'package:rentflow_api/src/model/health_response.dart';
import 'package:rentflow_api/src/model/import_avito_items_request.dart';
import 'package:rentflow_api/src/model/init_email_change_request.dart';
import 'package:rentflow_api/src/model/init_phone_change_request.dart';
import 'package:rentflow_api/src/model/integration_controller_get_feed_url200_response.dart';
import 'package:rentflow_api/src/model/invite_sub_account_request.dart';
import 'package:rentflow_api/src/model/invoice_item_input_dto.dart';
import 'package:rentflow_api/src/model/invoice_item_response.dart';
import 'package:rentflow_api/src/model/invoice_response.dart';
import 'package:rentflow_api/src/model/list_dashboard_events_response.dart';
import 'package:rentflow_api/src/model/list_documents_response.dart';
import 'package:rentflow_api/src/model/list_invoices_response.dart';
import 'package:rentflow_api/src/model/list_properties_response.dart';
import 'package:rentflow_api/src/model/list_rentals_response.dart';
import 'package:rentflow_api/src/model/list_signatures_response.dart';
import 'package:rentflow_api/src/model/list_template_authors_response.dart';
import 'package:rentflow_api/src/model/list_template_history_response.dart';
import 'package:rentflow_api/src/model/list_template_versions_response.dart';
import 'package:rentflow_api/src/model/list_templates_response.dart';
import 'package:rentflow_api/src/model/list_transactions_response.dart';
import 'package:rentflow_api/src/model/mark_deal_message_read_request.dart';
import 'package:rentflow_api/src/model/patch_user_request.dart';
import 'package:rentflow_api/src/model/property_additional_service_input_dto.dart';
import 'package:rentflow_api/src/model/property_additional_service_response.dart';
import 'package:rentflow_api/src/model/property_image_input_dto.dart';
import 'package:rentflow_api/src/model/property_image_response.dart';
import 'package:rentflow_api/src/model/property_manager_response.dart';
import 'package:rentflow_api/src/model/property_rent_terms_input_dto.dart';
import 'package:rentflow_api/src/model/property_rent_terms_response.dart';
import 'package:rentflow_api/src/model/property_response.dart';
import 'package:rentflow_api/src/model/property_utility_input_dto.dart';
import 'package:rentflow_api/src/model/property_utility_response.dart';
import 'package:rentflow_api/src/model/publication_import_response_dto.dart';
import 'package:rentflow_api/src/model/publish_listing_request.dart';
import 'package:rentflow_api/src/model/rental_response.dart';
import 'package:rentflow_api/src/model/reorder_images_request.dart';
import 'package:rentflow_api/src/model/send_otp_request.dart';
import 'package:rentflow_api/src/model/start_import_request.dart';
import 'package:rentflow_api/src/model/task_binding_dto.dart';
import 'package:rentflow_api/src/model/telegram_finalize_request.dart';
import 'package:rentflow_api/src/model/telegram_verify_request.dart';
import 'package:rentflow_api/src/model/template_author_response.dart';
import 'package:rentflow_api/src/model/template_block_input_dto.dart';
import 'package:rentflow_api/src/model/template_block_response.dart';
import 'package:rentflow_api/src/model/template_history_entry_response.dart';
import 'package:rentflow_api/src/model/template_version_response.dart';
import 'package:rentflow_api/src/model/transaction_response.dart';
import 'package:rentflow_api/src/model/update_account_type_request.dart';
import 'package:rentflow_api/src/model/update_additional_service_request.dart';
import 'package:rentflow_api/src/model/update_contact_request.dart';
import 'package:rentflow_api/src/model/update_contact_status_request.dart';
import 'package:rentflow_api/src/model/update_deal_meeting_request.dart';
import 'package:rentflow_api/src/model/update_deal_request.dart';
import 'package:rentflow_api/src/model/update_deal_stage_request.dart';
import 'package:rentflow_api/src/model/update_deal_status_request.dart';
import 'package:rentflow_api/src/model/update_deal_task_request.dart';
import 'package:rentflow_api/src/model/update_document_request.dart';
import 'package:rentflow_api/src/model/update_document_status_request.dart';
import 'package:rentflow_api/src/model/update_invoice_request.dart';
import 'package:rentflow_api/src/model/update_property_request.dart';
import 'package:rentflow_api/src/model/update_property_status_request.dart';
import 'package:rentflow_api/src/model/update_rental_request.dart';
import 'package:rentflow_api/src/model/update_rental_status_request.dart';
import 'package:rentflow_api/src/model/update_sub_account_request.dart';
import 'package:rentflow_api/src/model/update_task_request.dart';
import 'package:rentflow_api/src/model/update_telegram_request.dart';
import 'package:rentflow_api/src/model/update_template_request.dart';
import 'package:rentflow_api/src/model/upload_company_document_request.dart';
import 'package:rentflow_api/src/model/upsert_automation_request.dart';
import 'package:rentflow_api/src/model/upsert_company_profile_request.dart';
import 'package:rentflow_api/src/model/upsert_contact_kyc_request.dart';
import 'package:rentflow_api/src/model/upsert_rent_terms_request.dart';
import 'package:rentflow_api/src/model/upsert_utility_request.dart';
import 'package:rentflow_api/src/model/verify_otp_request.dart';

part 'serializers.g.dart';

@SerializersFor([
  AddImageRequest,
  ClientFlowMetricsResponse,
  CompleteAvitoOauthRequest,
  CompleteImportRequest,
  ConfirmEmailChangeRequest,
  ConfirmPhoneChangeRequest,
  ConnectPlatformRequest,
  ConnectionResponse,
  CreateAdditionalServiceRequest,
  CreateAuthorizationRequest,
  CreateContactNoteRequest,
  CreateContactRequest,
  CreateDealMeetingRequest,
  CreateDealMessageRequest,
  CreateDealNoteRequest,
  CreateDealRequest,
  CreateDealTaskRequest,
  CreateDocumentRequest,
  CreateInvoiceRequest,
  CreateNoteRequest,
  CreatePropertyRequest,
  CreateRentalRequest,
  CreateSessionRequest,
  CreateSignatureRequest,
  CreateStudioLeadRequest,
  CreateTaskRequest,
  CreateTemplateRequest,
  CreateTransactionRequest,
  DashboardEventEntityResponse,
  DashboardEventResponse,
  DocumentResponse,
  DocumentSignatureResponse,
  DocumentTemplateResponse,
  ExchangeTokenRequest,
  ExternalListingResponse,
  FinanceMetricsResponse,
  GenerateDocumentUploadUrlRequest,
  GenerateDocumentUploadUrlResponse,
  GenerateUploadUrlBodyDto,
  GetMeResponse,
  HealthResponse,
  ImportAvitoItemsRequest,
  InitEmailChangeRequest,
  InitPhoneChangeRequest,
  IntegrationControllerGetFeedUrl200Response,
  InviteSubAccountRequest,
  InvoiceItemInputDto,
  InvoiceItemResponse,
  InvoiceResponse,
  ListDashboardEventsResponse,
  ListDocumentsResponse,
  ListInvoicesResponse,
  ListPropertiesResponse,
  ListRentalsResponse,
  ListSignaturesResponse,
  ListTemplateAuthorsResponse,
  ListTemplateHistoryResponse,
  ListTemplateVersionsResponse,
  ListTemplatesResponse,
  ListTransactionsResponse,
  MarkDealMessageReadRequest,
  PatchUserRequest,
  PropertyAdditionalServiceInputDto,
  PropertyAdditionalServiceResponse,
  PropertyImageInputDto,
  PropertyImageResponse,
  PropertyManagerResponse,
  PropertyRentTermsInputDto,
  PropertyRentTermsResponse,
  PropertyResponse,
  PropertyUtilityInputDto,
  PropertyUtilityResponse,
  PublicationImportResponseDto,
  PublishListingRequest,
  RentalResponse,
  ReorderImagesRequest,
  SendOtpRequest,
  StartImportRequest,
  TaskBindingDto,
  TelegramFinalizeRequest,
  TelegramVerifyRequest,
  TemplateAuthorResponse,
  TemplateBlockInputDto,
  TemplateBlockResponse,
  TemplateHistoryEntryResponse,
  TemplateVersionResponse,
  TransactionResponse,
  UpdateAccountTypeRequest,
  UpdateAdditionalServiceRequest,
  UpdateContactRequest,
  UpdateContactStatusRequest,
  UpdateDealMeetingRequest,
  UpdateDealRequest,
  UpdateDealStageRequest,
  UpdateDealStatusRequest,
  UpdateDealTaskRequest,
  UpdateDocumentRequest,
  UpdateDocumentStatusRequest,
  UpdateInvoiceRequest,
  UpdatePropertyRequest,
  UpdatePropertyStatusRequest,
  UpdateRentalRequest,
  UpdateRentalStatusRequest,
  UpdateSubAccountRequest,
  UpdateTaskRequest,
  UpdateTelegramRequest,
  UpdateTemplateRequest,
  UploadCompanyDocumentRequest,
  UpsertAutomationRequest,
  UpsertCompanyProfileRequest,
  UpsertContactKycRequest,
  UpsertRentTermsRequest,
  UpsertUtilityRequest,
  VerifyOtpRequest,
])
Serializers serializers = (_$serializers.toBuilder()
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ExternalListingResponse)]),
        () => ListBuilder<ExternalListingResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ConnectionResponse)]),
        () => ListBuilder<ConnectionResponse>(),
      )
      ..add(const OneOfSerializer())
      ..add(const AnyOfSerializer())
      ..add(const DateSerializer())
      ..add(Iso8601DateTimeSerializer()))
    .build();

Serializers standardSerializers =
    (serializers.toBuilder()..addPlugin(StandardJsonPlugin())).build();
